...
package langotec.numberq.store.map;
...
public class PhpDB extends Application implements Runnable {
    ...
    public class ItemListRow {
        ...
        public Object get(String key) {
            return subItem.get(key);
        }

        public Object getkeySet() {
            return subItem.keySet();
        }
    ...
    public String getPairFunction() {
        if (pairSet != null) {
            return pairSet.getPairFunction();
        } else return "";
    }
    ...
    //傳回處理完的SQL資料
    public ArrayList<ItemListRow> getDataSet() {
        if (blReady) return itemListSet;
        else return null;
    }

    //回傳JSON
    public JSONArray getJSONData() {
        if (pairSet.isJSON()) return jsonArray;
        else return null;
    }

    //傳回資料筆數
    public int getRowSize() {
        if (blReady) return itemListSet.size();
        else return -1;
    }

    public int getColSize() {
        if (blReady) return ((ItemListRow) itemListSet.get(0)).size();
        else return -1;
    }
    ...
    public void setPairSet(PairSet tpairSet) {
        pairSet = tpairSet;
    }

    //返回是否JSON
    public boolean isJSON() {
        return pairSet.isJSON();
    }
    ...
    public class PairSet {
        ...
        final String phpSQLnewOrderSub = context.getResources().getString(R.string.phpSQLnewOrderSub); //訂單菜單新增
        ...
        public PairSet(double lat, double lng) {
            this("", true, -1, lat, lng);
        }
            ...
            try {
                if (pairs.size() > 0) {
                    for (int i = 0; i < pairs.size(); i++) {
                        if (pairs.get(i).getName().equals(workName)) pairs.remove(i);
                    }
                }
                pairs.add(new BasicNameValuePair(workName, value));
            } catch (Exception ex) {
                Log.e("參數設定發生錯誤:", ex.toString());
                ex.printStackTrace();
            }
        }
        ...
        public void removePair(String pName) {
            ...
            try {
                if (pairs.size() > 0) {
                    for (int i = 0; i < pairs.size(); i++) {
                        if (pairs.get(i).getName().equals(pName)) pairs.remove(i);
                    }
                }
            } catch (Exception ex) {
                Log.e("參數移除發生錯誤:", ex.toString());
                ex.printStackTrace();
            }
        }
    ...
    public class HttpDataFromPHP extends Thread {
        ...
        //建構子
        public HttpDataFromPHP() {
            this(null, null);
        }

        public HttpDataFromPHP(Handler hand) {
            this(hand, null);
        }

        public HttpDataFromPHP(PairSet tpairSet) {
            this(null, tpairSet);
        }
        ...
        private HttpResponse getPHPConnection(String serverURL, String fileURL, String pairWork) {
            ...
            try {
                if (androidHttpClient == null)
                    androidHttpClient = AndroidHttpClient.newInstance(System.getProperty("http_agent"));
                httppost = new HttpPost(serverURL + fileURL);

                //參數選擇 空的就使用預設值(查詢)
                if (pairWork.equals("")) {
                    pairSet.setPairDefault();
                } else {
                    pairSet.setPairFunction(pairWork);
                }
                ...
                if (status.split(" ")[1].equals("200")) {
                    ...
                    while (((readLine = br.readLine()) != null)) {
                        ...
                        for (int i = 0; i < item_String.length; i++) {
                            if (visTitle == true) {
                                itemListRow.add(item_Field_String[i], item_String[i]);
                            } else {
                                itemListRow.add("" + i, item_String[i]);
                            }
                        }
